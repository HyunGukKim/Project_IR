...
public abstract class GameData : Normal
{
    ...
    public DateTime lastBackendUploadDate = DateTime.MinValue;
    ...
    public abstract void InitializeData(); // 자식 객체가 설정한 데이터 초기화를 해주는 함수

    protected abstract void SetServerDataToLocal(JsonData gameDataJson); // 자식 객체가 설정한 불러오기 함수 호출 이후 각 테이블에 맞게 파싱하는 함수

    public abstract void LocalSave(); // 로컬에 데이터 저장하기 단순웨이브 클리어 정도는 로컬에 저장한다
    public abstract void LocalLoad(); // 로컬 데이터 가져오기 
            ...
            try
            {
                ...
                if (callback.IsSuccess())
                {
                    ...
                    } else
                    {
                        ...
                        // 데이터를 새로 생성
                        Debug.Log("데이터가 존재하지 않아 새로 생성합니다.");
    ...
    private void BackendInsert(AfterBackendLoadFunc afterBackendLoadFunc)
    {
        ...
        // [뒤끝] 게임 정보 삽입 함수
        Debug.Log($"Backend.GameData.Insert({tableName})");
        Debug.Log(GetParam()); 
        Backend.GameData.Insert(tableName, GetParam(), callback => {
            try
            {
    ...
    {
        lastBackendUploadDate = DateTime.Now;
        Backend.GameData.UpdateV2(GetTableName(), GetInDate(), Backend.UserInDate, GetParam(),
            callback => {
